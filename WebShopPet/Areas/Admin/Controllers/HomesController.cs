    ...
    public class HomesController : BaseController
    {
        ...
        public ActionResult Index(string startDate, string endDate)
        {
            ...
            if (!String.IsNullOrEmpty(startDate) && !String.IsNullOrEmpty(endDate))
            {
                ...
                ViewBag.TienThu = db.ORDER_DETAILS.Where(x => x.ORDER.DATE >= date1).Where(x => x.ORDER.DATE <= date2).Sum(x => (x.PRODUCT.PRICE * x.QUANTITY - x.PRODUCT.PRICE * x.QUANTITY * x.PRODUCT.DISCOUNT /100)*1000);

                ViewBag.TienLai = db.ORDER_DETAILS.Where(x => x.ORDER.DATE >= date1).Where(x => x.ORDER.DATE <= date2).Sum(x => (x.PRODUCT.PRICE * x.QUANTITY - x.PRODUCT.PRICE * x.QUANTITY * x.PRODUCT.DISCOUNT /100)*1000) - db.ORDER_DETAILS.Where(x => x.ORDER.DATE >= date1).Where(x => x.ORDER.DATE <= date2).Sum(x => (x.PRODUCT.IMPORT_PRICE * x.QUANTITY)*1000);
